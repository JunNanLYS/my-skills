...
| S15.1 | B | non-empty canvas/list/nodes collection output → Workflow 9 FAIL → enter Workflow 10 correction loop |
| S15.2 | B | non-empty `read arrange --dry-run` → Workflow 9 FAIL → executor-computed next coordinates + rerun --dry-run |
| S15.3 | B | `read arrange --apply` overlap count > 0 → Workflow 9 FAIL → `figma-cli pos <id> --x <x> --y <y>` + rerun read arrange |
