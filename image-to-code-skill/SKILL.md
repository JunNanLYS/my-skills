---
name: image-to-code
description: 网站图片转代码的首选技能。Use when 用户要给一张或一组视觉稿/参考图，转成生产级前端代码；要求先自行生成/分析设计稿、然后实现到大屏小屏都可读的网页。强调：先生成大尺寸分区级图像而不是压缩小图；为段落与细节单独出新图而不是裁切旧图；禁止卡片套卡片套卡片的 UI；保证 Hero 干净、宽敞、可读且在小屏笔记本上完整可见。适用于视觉优先的网页任务，不适用于非网页类 UI（原生桌面/移动 App）。
---
