...
description: 反 slop 前端技能，专攻落地页、作品集、改版翻新。Use when 用户要做面向消费者的网页/产品页/设计师作品集/品牌改版，并希望避免 AI 千篇一律的模板感。流程：先读简报推断设计方向，再用真实设计系统（Fluent UI、Material、Carbon、Polaris、Primer、shadcn/ui、Tailwind 等），改版场景必须先审计再改；内置严格起飞前自检（em-dash 禁用、Hero 容量上限、调色一致性、按钮对比度、eyebrow 上限等）。
---
