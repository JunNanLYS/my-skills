...
| 参数名 | 类型 | 必填 | 示例值 | 说明 |
|--------|------|------|--------|------|
| `model_key` | string | 是 | `background-removal` | 固定值 |
| `image_file` | file | 是 | `@"/path/to/image"` | 上传的图片文件 |
| `output_format` | string | 是 | `webp` | 输出图片格式 |
| `crop` | string | 是 | `0` | 是否裁剪（0=否，1=是） |
| `border` | string | 是 | `0` | 是否添加边框（0=否，1=是） |
| `stamp_crop` | string | 是 | `0` | 是否印花裁剪（0=否，1=是） |
...
--form 'model_key="background-removal"' \
